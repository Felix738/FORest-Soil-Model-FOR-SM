...
subroutine s_brook90_f( siteparam, climveg, param, pdur, soil_materials, soil_nodes, precdat, &
    ...
    ! FS: declare new variable read in from FORMIND output file
    real :: ATRANI_FORMIND    ! Declare the variable actual transpiration from FORMIND
    real :: INTER_FORMIND     ! Declare the variable interception from FORMIND

    ! Input of parameters
    real(kind=c_double), dimension(85), intent(in) :: param
    ...
    ! Output matrix
    real(kind=c_double), dimension( INT(param(1) * siteparam(6)), 49), intent(inout) :: output_day
...
!        actual transpiration and ground evaporation rates
            IF (PTR(J) .GT. .0010d0) THEN
                CALL TBYLAYER(J, PTR(J), DISPC, ALPHA, KK, RROOTI, RXYLEM, &
                    PSITI, NLAYER, PSICR, NOOUTF, ATR(J), ATRANI, param)
...
!     * * * * * * * * *  E N D   D A Y - N I G H T   L O O P  * * * * * * * * * * * * * *
		...
		! FS: exchange evapotranspiration with newly set parameter from the FORMIND model
...
!           no snow, soil evaporation weighted for WETFR
                        if(PsiM(1).gt.PsiCrit(1)) then
                            ...
                            ! SLVP = WETFR * GIVP + (1.0d0 - WETFR) * GEVP
							!========================================================
							! FORMIND controls evapotranspiration
							!========================================================
							...
							DO I = 1, NLAYER
							 	TRANI(I) = param(85) * RELDEN(I) * 4.4d0
							 	! TRANI(I) = param(85) / NLAYER
							END DO
...
! Errechnet die vertikalen FlÃ¼sse von Wasser zwischen den einzelnen Schichten.
...
! angenommen (vertical_flow_mode = 1).
    ...
    select case(gwv%vertical_flow_mode)
        ...
        case(0)! Versickerung am unteren Rand. Potentialgradient Ã¼ber DRAIN einstellbar

        ! Errechnet vertikale FlÃ¼sse in den ZWISCHEN den Schichten 1 bis NLAYER
        do i = 1, NLAYER-1
        ...
        case(1)! Grundwasser im Profil. Grundwasserstand Ã¼ber water_table_depth einstellbar.

        if (gwv%N_groundwater <= NLAYER) then! Grundwasserstand innerhalb der Schichten
            ! Errechne vertikale FlÃ¼sse zwischen den Schichten 1 bis N_groundwater (in letzterer
            ! steht irgendwo das Grundwasser)
            do i = 1, gwv%N_groundwater-1
                call VERT(KK(i), KK(i + 1),Par6(i),Par6(i+1), THICK(i), THICK(i + 1), &
                         PSITI(i), PSITI(i + 1), STONEF(i), STONEF(i + 1), RHOWG, VRFLI(i))
            end do
            ! Setzt die vertikalen FlÃ¼sse an den Schichtgrenzen, auf einen erkennbar noch zu Ã¤ndernden
            ! Wert. Die Ã„nderung sollte in calc_inflows geschehen (bzw. an der Stelle VRFLI = VV im Text).
            VRFLI(gwv%N_groundwater:NLAYER) = 8888.d0! VRFLI(gwv%N_groundwater-1)

        else! Grundwasserstand unterhalb aller Schichten
            ! Errechne vertikale FlÃ¼sse zwischen den Schichten 1 bis NLAYER
            do i = 1, NLAYER-1
                call VERT(KK(i), KK(i + 1),Par6(i),Par6(i+1), THICK(i), THICK(i + 1), &
                         PSITI(i), PSITI(i + 1), STONEF(i), STONEF(i + 1), RHOWG, VRFLI(i))
            end do
            ! Nimm eine imaginÃ¤re Schicht unter allen Schichten an, die so dick ist, dass ihre
            ! Mitte den Grundwasserspiegel trifft.
            ! LeitfÃ¤higkeit, gesÃ¤ttigte LeitfÃ¤higkeit und Steingehalt gleichen der Schicht NLAYER.
        ...
        case default
        if (.true.) call intpr("Den Case fÃ¼r die untere Randbedingung gibt es nicht in calc_vertical_flows.", -1, (/-0/),0)
        error = 9
...
subroutine calc_inflows(gwv, NLAYER, DTI, INFRAC, BYFRAC, SLFL, VRFLI, DSFLI, TRANI, SLVP, SWATMX, SWATI,&
    ...
    select case(gwv%vertical_flow_mode)
        ...
        ! Errechne VV, INFLI, BYFLI, NTFLI nur fÃ¼r die Schichten oberhalb N_groundwater
        ...
        ! Errechne VV, INFLI, BYFLI, NTFLI fÃ¼r die Schichten im Grundwasser
        ! Insbesondere VV ist unphysikalisch, weil es so gewÃ¤hlt wird, dass bei gegebenen INFLI, DSFLI und TRANI
        ! (und SLVP) der Nettofluss NTFLI verschwindet und somit der Annahme voller SÃ¤ttigung zumindest insoweit
        ! GenÃ¼ge getan wird.
        if (gwv%N_groundwater <= NLAYER) then
            ! Weil gesÃ¤ttigt, sollte von BYFLFR entweder BYFRAC = 1 ausgerechnet werden, wenn BYPAR = 1, und ansonsten 0
        ...
        if (pr == 1) call intpr("Den Case fÃ¼r die untere Randbedingung gibt es nicht in calc_inflows.", -1, (/-0/),0)
        error = 9
...
!   Output
    ...
    select case(gwv%vertical_flow_mode)
        ...
        case default
        if (pr == 1) call intpr("Den Case fÃ¼r die untere Randbedingung gibt es nicht in calc_DTINEW.", -1, (/-0/),0)
        error = 9
...
!     input
    ...
    real(kind=c_double) :: BYFRAC(*) ! fraction of layer infiltration to bypass flow
    real(kind=c_double) :: SLFL      ! input rate to soil surface, mm/d
    real(kind=c_double) :: DSFLI(*)  ! downslope flow rate 	
    real(kind=c_double) :: ATRANI_FORMIND(1) ! single transpiration value, mm/d
    real(kind=c_double) :: TRANI(*)  ! transpiration rate from layer, mm/d
    real(kind=c_double) :: SLVP      ! evaporation rate from soil, mm/d
    real(kind=c_double) :: SWATMX(*) ! maximum water storage for layer, mm
    real(kind=c_double) :: SWATI(*)  ! water volume in layer, mm
...
!     local
    ...
    ! Initialize the TRANI array with the repeated value of ATRANI_FORMIND
    !do I = 1, NLAYER
    !    TRANI(I) = ATRANI_FORMIND
    !end do
...
! FS use params
subroutine TBYLAYER (J, PTR, DISPC, ALPHA, KK, RROOTI, RXYLEM, &
    PSITI, NLAYER, PSICR, NOOUTF, ATR, ATRANI, param)
...
!           daytime, PTR is average of a half sine over daytime
            ...
            ELSEIF (R .LT. 1.0d0) THEN
				...
				! FS: use transpiration rates from FORMIND
				ATR = param( 85 )
				! ATR = 0.5d0
            ELSE
                ! ATR = PTR
